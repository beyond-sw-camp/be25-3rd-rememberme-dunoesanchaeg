...
// 최근 7회차라고 요구사항에 명시되어 있으므로, mockApiResponse의 단일 accuracy를 목표로 무작위 가짜 변동선을 생성해 줍니다.
const trendHistory = ref([]);

const generateTrendData = (targetAccuracy) => {
  // 1회차부터 7회차까지의 데이터를 생성하여 우상향/변동 하는 그럴싸한 그래프 생성
  const dummy = [];
  let currentVal = Math.max(0, targetAccuracy - 20); // 20점 밑에서 대충 시작
  for(let i = 1; i <= 7; i++) {
    if(i === 7) {
...
const computedChartData = computed(() => {
  // Pretendard 폰트가 전역 설정되어 있지만, 인라인 스타일도 안전하게 보조하기 위해 색상 변수를 직접 씀
    ...
    datasets: [
      {
        ...
        backgroundColor: 'rgba(45, 122, 54, 0.15)', // 투명한 초록
        borderWidth: 3,
        ...
        pointHoverRadius: 7,
        fill: true,
        tension: 0.4 // 부드러운 곡선(애니메이션 효과 극대화)
      }
    ]
  };
});
...
<style scoped>
/* Chart.js 캔버스는 wrapper 크기에 맞게 채워집니다. */
</style>
